...
# Priority order of working models for quota distribution & fast failover
_FALLBACK_MODELS = [
    "gemini-3.6-flash",
    "gemini-3.7-flash",
    "gemini-3.5-flash",
...
def complete(user_text: str, system_message: str = "") -> str | None:
    ...
    if _GEMINI_KEY and _GEMINI_KEY != "dummy":
        ...
        # Fast failover across available models
        for model_name in _FALLBACK_MODELS:
            try:
                ...
                if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                    logger.warning(f"Gemini {model_name} rate limit reached, trying next model.")
                elif "503" in err_str or "UNAVAILABLE" in err_str:
                    logger.warning(f"Gemini {model_name} 503 unavailable, trying next model.")
                elif "404" in err_str or "NOT_FOUND" in err_str:
                    logger.warning(f"Gemini {model_name} not available (404), trying next model.")
                else:
